...
// Function to set baudrate for UART0
void set_baudrate(int baudrate) {
    unsigned int baud_reg_value = (4000000 / (baudrate * 16)) - 1;  // Assuming UART_CLOCK = 4MHz
    UART0_IBRD = baud_reg_value / 64;  // Integer part
    UART0_FBRD = baud_reg_value % 64;  // Fractional part
    uart_puts("UART0 baud rate set to ");
    uart_dec(baudrate);
    uart_puts(" bps\n");
}
void uart_delete() {
...
// Function to set stop bits for UART0
void uart0_set_stopbit(int stop_bits) {
    UART0_CR &= ~UART0_CR_UARTEN; // Disable UART0 temporarily

    if (stop_bits == 1) {
        UART0_LCRH &= ~UART0_LCRH_STP2; // Clear STP2 bit to set 1 stop bit
        uart_puts("UART0 stop bit set to 1\n");
    } else if (stop_bits == 2) {
        UART0_LCRH |= UART0_LCRH_STP2; // Set STP2 bit to enable 2 stop bits
        uart_puts("UART0 stop bit set to 2\n");
    } else {
        uart_puts("Invalid stop bit setting. Use 1 or 2.\n");
    }

    UART0_CR |= UART0_CR_UARTEN; // Re-enable UART0
}
// Receive a character
char new_uart_getc() {
    char c;
